...
///   - Raftが正しく動作するためには、このストレージは完全に信頼できるものである必要がある
...
pub trait Io {
    ...
    ///
    /// Ok(Some(message)) ==> messageを受信した
    /// Ok(None) => 何も受信していない
    /// Err => エラーが発生した。
    ///
    /// エラーのsemantics:
    /// このレイヤでのエラー発生は、致命的で自動解決が不能なことを意味する。
    /// よって、エラーはローカルのRaftノードを停止させる。
    /// 時間経過で解決する(None => Someになる可能性がある)うちはエラーを返すべきではない。
    ...
    ///
    /// 仮にチャンネルの致命的な問題が発生している場合にローカルノードを停止したいならば、
    /// 以降の`try_recv_message`メソッド呼び出しで`Err`を返すこと.
    ...
    /// ローカルログの指定範囲のエントリ (start..end) を取得する.
    ///

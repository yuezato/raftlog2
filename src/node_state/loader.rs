        ...
        while let Async::Ready(phase) = track!(self.phase.poll().map_err(Error::from))? {
            let next = match phase {
                ...
                Phase::B(log) => {
                    // 2) ローカルログの内容を復元
                    //
                    // FIX: 何もシリアライズしているデータがない場合に
                    // 現在だとクラッシュしてしまうが、本来はどうするつもりだったのか。
                    match log {
                        Log::Prefix(prefix) => {
                            // 2-1) スナップショットを読み込んだ

                            // これhistoryはシリアライズされてないので、
                            // そのことを思い出そうとすると
                            // 本来snapshotをシリアライズする時に
                            // 行うことを、無理やりここでやることになるのかな
                            track!(common.handle_log_snapshot_installed(
                                prefix.tail,
                                prefix.config.clone(),
                            ))?;

                            // loadイベントをhistoryに残すというのは、まだ分かる
                            // とはいえ何故必要なのかまるで分からないのでやめて欲しい
                            track!(common.handle_log_snapshot_loaded(prefix))?;

                            // このlogというのは何なのかと思えば、historyのことらしい

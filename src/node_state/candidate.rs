...
/// - 2. 投票依頼をブロードキャスト（注意: 自己投票とbroadcastはタイムアウトに関係なく即座に行う）
...
impl<IO: Io> Candidate<IO> {
    ...
    pub fn handle_message(
        ...
        if let Message::RequestVoteReply(RequestVoteReply { voted: true, .. }) = message {
            ...
            // common.config()は「恐らく」現在commitしているconfiguration
            // λ(n: node). followers.contains(n) は nが自分にvoted => true, non-voted => false な関数
